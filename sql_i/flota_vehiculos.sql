-- Práctica Laura Aparicio García
...
INSERT INTO flota_vehiculos.coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
create table flota_vehiculos.polizas_seguro(
...
create table flota_vehiculos.aseguradoras(
...
alter table flota_vehiculos.vehiculos add constraint fk_vehiculos_colores foreign key (id_color) references flota_vehiculos.colores(id_color);
alter table flota_vehiculos.vehiculos add constraint fk_vehiculos_modelos foreign key (id_modelo) references flota_vehiculos.modelos(id_modelo);

alter table flota_vehiculos.modelos add constraint fk_modelos_marcas foreign key (id_marca) references flota_vehiculos.marcas(id_marca);

alter table flota_vehiculos.marcas add constraint fk_marcas_grupos foreign key (id_grupo) references flota_vehiculos.grupo_empresarial(id_grupo);

alter table flota_vehiculos.polizas_seguro add constraint fk_polizas_vehiculos foreign key (id_vehiculo) references flota_vehiculos.vehiculos(id_vehiculo);
alter table flota_vehiculos.polizas_seguro add constraint fk_polizas_aseguradoras foreign key (id_aseguradora) references flota_vehiculos.aseguradoras(id_aseguradora);

alter table flota_vehiculos.revisiones add constraint fk_revisiones_vehiculos foreign key (id_vehiculo) references flota_vehiculos.vehiculos(id_vehiculo);
alter table flota_vehiculos.revisiones add constraint fk_revisiones_moneda foreign key (id_moneda) references flota_vehiculos.moneda(id_moneda);


-- Volcado de datos de coches a las diferentes entidades
insert into flota_vehiculos.grupo_empresarial(grupo)
			select
				grupo
			from flota_vehiculos.coches 
			group by grupo 
			order by grupo
;

insert into flota_vehiculos.marcas(marca,id_grupo)
			select
				coches.marca,
				grupo_empresarial.id_grupo
			from flota_vehiculos.coches 
			inner join flota_vehiculos.grupo_empresarial on grupo_empresarial.grupo = coches.grupo
			group by grupo_empresarial.id_grupo, coches.marca
			order by grupo_empresarial.id_grupo, coches.marca
;

insert into flota_vehiculos.modelos(modelo,id_marca)
			select
				coches.modelo,
				marcas.id_marca
			from flota_vehiculos.coches
			inner join flota_vehiculos.marcas on marcas.marca = coches.marca
			group by marcas.id_marca, coches.modelo
			order by marcas.id_marca, coches.modelo
;

insert into flota_vehiculos.colores(color)
			select
				coches.color
			from flota_vehiculos.coches
			group by color
...
insert into flota_vehiculos.vehiculos(matricula,fecha_compra,id_modelo,id_color,kilometraje_actual)
			select
				coches.matricula,
				coches.fecha_compra,
				modelos.id_modelo,
				...
				coches.kms_totales
			from flota_vehiculos.coches
			inner join flota_vehiculos.modelos on modelos.modelo = coches.modelo
			inner join flota_vehiculos.colores on colores.color = coches.color
			group by coches.matricula,coches.fecha_compra,modelos.id_modelo,colores.id_color,coches.kms_totales
			order by coches.matricula
;

insert into flota_vehiculos.aseguradoras(aseguradora)
			select
				aseguradora
			from flota_vehiculos.coches
...
insert into flota_vehiculos.polizas_seguro(referencia,fecha_alta,id_aseguradora,id_vehiculo)
			select
				coches.n_poliza,
				coches.fecha_alta_seguro,
				aseguradoras.id_aseguradora,
				vehiculos.id_vehiculo
			from flota_vehiculos.coches
			inner join flota_vehiculos.aseguradoras on aseguradoras.aseguradora = coches.aseguradora
			inner join flota_vehiculos.vehiculos on vehiculos.matricula = coches.matricula
			group by coches.n_poliza, coches.fecha_alta_seguro,aseguradoras.id_aseguradora,vehiculos.id_vehiculo
			order by coches.fecha_alta_seguro,aseguradoras.id_aseguradora,coches.n_poliza,vehiculos.id_vehiculo
;

insert into flota_vehiculos.moneda(tipo_moneda)
			select
				moneda
			from flota_vehiculos.coches
			group by moneda
...
insert into flota_vehiculos.revisiones(fecha_revision,kilometraje_revision,importe_revision,id_moneda,id_vehiculo)
			select
				coches.fecha_revision,
				coches.kms_revision,
				coches.importe_revision,
				moneda.id_moneda,
				vehiculos.id_vehiculo
			from flota_vehiculos.coches
			inner join flota_vehiculos.vehiculos on vehiculos.matricula = coches.matricula
			inner join flota_vehiculos.moneda on tipo_moneda = coches.moneda
			group by coches.fecha_revision,coches.kms_revision,coches.importe_revision,moneda.id_moneda,vehiculos.id_vehiculo
			order by coches.fecha_revision,vehiculos.id_vehiculo
;


-- Borrado de la tabla con los datos de la práctica
drop table flota_vehiculos.coches
;


-- Consulta final práctica
select distinct
	vehiculos.matricula,
	modelos.modelo,
	marcas.marca,
	...
	polizas_seguro.referencia		as	numero_poliza,
	aseguradoras.aseguradora
from flota_vehiculos.vehiculos
join flota_vehiculos.colores						on	vehiculos.id_color				=	colores.id_color
join flota_vehiculos.modelos						on	vehiculos.id_modelo				=	modelos.id_modelo
join flota_vehiculos.marcas							on	modelos.id_marca				=	marcas.id_marca
join flota_vehiculos.grupo_empresarial				on	marcas.id_grupo					=	grupo_empresarial.id_grupo
join (select
				max(fecha_alta)		as fecha_poliza, 
				id_vehiculo 
			from flota_vehiculos.polizas_seguro 
			group by id_vehiculo )	as ult_poliza	on	vehiculos.id_vehiculo			=	ult_poliza.id_vehiculo
join flota_vehiculos.polizas_seguro					on	polizas_seguro.id_vehiculo		=	ult_poliza.id_vehiculo
													and polizas_seguro.fecha_alta		=	ult_poliza.fecha_poliza
join flota_vehiculos.aseguradoras					on	polizas_seguro.id_aseguradora	=	aseguradoras.id_aseguradora

order by vehiculos.matricula asc
;